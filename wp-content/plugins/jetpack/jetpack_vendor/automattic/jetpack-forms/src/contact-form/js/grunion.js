...
GrunionFB_i18n=jQuery.extend({nameLabel:'Name',emailLabel:'Email',urlLabel:'Website',commentLabel:'Comment',newLabel:'New Field',optionsLabel:'Options',optionLabel:'Option',firstOptionLabel:'First option',problemGeneratingForm:"Oops, there was a problem generating your form.  You'll likely need to try again.",moveInstructions:'Drag up or down\nto re-arrange',moveLabel:'move',editLabel:'edit',savedMessage:'Saved successfully',requiredLabel:'(required)',exitConfirmMessage:'Are you sure you want to exit the form editor without saving?  Any changes you have made will be lost.',maxNewFields:5,invalidEmail:' is an invalid email address.',},GrunionFB_i18n);GrunionFB_i18n.moveInstructions=GrunionFB_i18n.moveInstructions.replace('\n','<br />');FB.span=jQuery('<span>');FB.esc_html=function(string){return FB.span.text(string).html();};FB.esc_attr=function(string){string=FB.esc_html(string);return string.replace('"','&quot;').replace("'",'&#039;');};FB.ContactForm=(function(){const fbForm={action:'grunion_shortcode',_ajax_nonce:ajax_nonce_shortcode,to:'',subject:'',fields:{},};const defaultFields={name:{label:GrunionFB_i18n.nameLabel,type:'name',required:true,options:[],order:'1',},email:{label:GrunionFB_i18n.emailLabel,type:'email',required:true,options:[],order:'2',},url:{label:GrunionFB_i18n.urlLabel,type:'url',required:false,options:[],order:'3',},comment:{label:GrunionFB_i18n.commentLabel,type:'textarea',required:true,options:[],order:'4',},};const debug=false;let grunionNewCount=0;const maxNewFields=GrunionFB_i18n.maxNewFields;let optionsCache={};let optionsCount=0;let shortcode;function addField(){try{grunionNewCount++;if(grunionNewCount<=maxNewFields){jQuery('#fb-extra-fields').append('<div id="fb-new-field'+
grunionNewCount+'" fieldid="'+
grunionNewCount+'" class="fb-new-fields"><div class="fb-fields"><div id="'+
grunionNewCount+'" class="fb-remove"></div><label fieldid="'+
grunionNewCount+'" for="fb-field'+
grunionNewCount+'"><span class="label-text">'+
GrunionFB_i18n.newLabel+'</span> </label><input type="text" id="fb-field'+
grunionNewCount+'" disabled="disabled" /></div></div>');fbForm.fields[grunionNewCount]={label:GrunionFB_i18n.newLabel,type:'text',required:false,options:[],order:'5',};if(grunionNewCount===maxNewFields){jQuery('#fb-new-field').hide();}
optionsCount=0;optionsCache={};jQuery('#fb-new-options').html('<label for="fb-option0">'+
GrunionFB_i18n.optionsLabel+'</label><input type="text" id="fb-option0" optionid="0" value="'+
GrunionFB_i18n.firstOptionLabel+'" class="fb-options" />');jQuery('#fb-options').hide();jQuery('#fb-new-label').val(GrunionFB_i18n.newLabel);jQuery('#fb-new-type').val('text');jQuery('#fb-field-id').val(grunionNewCount);setTimeout(function(){jQuery('#fb-new-label').focus().select();},100);}else{jQuery('#fb-new-field').hide();}}catch(e){if(debug){console.log('addField(): '+e);}}}
function addOption(){try{optionsCount=jQuery('#fb-new-options .fb-options').length;const thisId=jQuery('#fb-field-id').val();const thisType=jQuery('#fb-new-type').val();if(thisType==='radio'){jQuery('#fb-new-options').append('<div id="fb-option-box-'+
optionsCount+'" class="fb-new-fields"><span optionid="'+
optionsCount+'" class="fb-remove-option"></span><label></label><input type="text" id="fb-option'+
optionsCount+'" optionid="'+
optionsCount+'" value="'+
GrunionFB_i18n.optionLabel+'" class="fb-options" /><div>');jQuery('#fb-new-field'+thisId+' .fb-fields').append('<div id="fb-radio-'+
thisId+'-'+
optionsCount+'"><input type="radio" disabled="disabled" id="fb-field'+
thisId+'" name="radio-'+
thisId+'" /><span>'+
GrunionFB_i18n.optionLabel+'</span><div class="clear"></div></div>');}else if('checkbox-multiple'===thisType){jQuery('#fb-new-options').append('<div id="fb-option-box-'+
optionsCount+'" class="fb-new-fields"><span optionid="'+
optionsCount+'" class="fb-remove-option"></span><label></label><input type="text" id="fb-option'+
optionsCount+'" optionid="'+
optionsCount+'" value="'+
GrunionFB_i18n.optionLabel+'" class="fb-options" /><div>');jQuery('#fb-new-field'+thisId+' .fb-fields').append('<div id="fb-checkbox-multiple-'+
thisId+'-'+
optionsCount+'"><input type="checkbox" disabled="disabled" id="fb-field'+
thisId+'" name="checkbox-multiple-'+
thisId+'" /><span>'+
GrunionFB_i18n.optionLabel+'</span><div class="clear"></div></div>');}else{jQuery('#fb-new-options').append('<div id="fb-option-box-'+
optionsCount+'" class="fb-new-fields"><span optionid="'+
optionsCount+'" class="fb-remove-option"></span><label></label><input type="text" id="fb-option'+
optionsCount+'" optionid="'+
optionsCount+'" value="" class="fb-options" /><div>');jQuery('#fb-field'+thisId).append('<option id="fb-'+
thisId+'-'+
optionsCount+'" value="'+
thisId+'-'+
optionsCount+'"></option>');}
...
jQuery.each(fbForm.fields,function(index,value){jQuery('#fb-extra-fields').before('<div class="fb-new-fields ui-state-default" fieldid="'+
index+'" id="fb-new-field'+
index+'"><div class="fb-fields"></div></div>');jQuery('#fb-field-id').val(index);optionsCache[index]={};optionsCache[index].options=[];if('radio'===value.type||'select'===value.type||'checkbox-multiple'===value.type){jQuery.each(value.options,function(i,v){optionsCache[index].options[i]=v;});}
updateType(value.type,value.label,value.required);});}catch(e){if(debug){console.log('buildPreview(): '+e);}}}
function customOptions(id,thisType){try{let thisOptions='';for(let i=0;i<optionsCache[id].options.length;i++){if(optionsCache[id].options[i]!==undefined){if(thisType==='radio'){thisOptions=thisOptions+'<div id="fb-radio-'+
id+'-'+
i+'"><input type="radio" id="fb-field'+
id+'" name="radio-'+
id+'" /><span>'+
FB.esc_html(optionsCache[id].options[i])+'</span><div class="clear"></div></div>';}else if('checkbox-multiple'===thisType){thisOptions=thisOptions+'<div id="fb-checkbox-multiple-'+
id+'-'+
i+'"><input type="checkbox" id="fb-field'+
id+'" name="checkbox-multiple-'+
id+'" /><span>'+
FB.esc_html(optionsCache[id].options[i])+'</span><div class="clear"></div></div>';}else{thisOptions=thisOptions+'<option id="fb-'+
id+'-'+
i+'" value="'+
id+'-'+
i+'">'+
FB.esc_html(optionsCache[id].options[i])+'</option>';}}}
return thisOptions;}catch(e){if(debug){console.log('customOptions(): '+e);}}}
function deleteField(that){try{grunionNewCount--;const thisId=that.attr('id');delete fbForm.fields[thisId];jQuery('#'+thisId).parent().parent().remove();if(grunionNewCount<=maxNewFields){jQuery('#fb-new-field').show();}}catch(e){if(debug){console.log('deleteField(): '+e);}}}
function editField(that){try{scroll(0,0);setTimeout(function(){jQuery('#fb-new-label').focus().select();},100);const thisId=that.parent().attr('fieldid');loadFieldEditor(thisId);}catch(e){if(debug){console.log('editField(): '+e);}}}
...
that.find('label').prepend('<span class="right fb-edit-field" style="font-weight: normal;"><a href="" class="fb-reorder"><div style="display: none;">'+
GrunionFB_i18n.moveInstructions+'</div>'+
GrunionFB_i18n.moveLabel+'</a>&nbsp;&nbsp;<span style="color: #C7D8DE;">|</span>&nbsp;&nbsp;<a href="" class="fb-edit">'+
GrunionFB_i18n.editLabel+'</a></span>');}else{jQuery('.fb-edit-field').remove();}}catch(e){if(debug){console.log('hideShowEditLink(): '+e);}}}
function loadFieldEditor(id){try{const thisType=fbForm.fields[id].type;jQuery('#fb-options').hide();jQuery('#fb-field-id').val(id);jQuery('#fb-new-label').val(fbForm.fields[id].label);jQuery('#fb-new-type').val(fbForm.fields[id].type);if(fbForm.fields[id].required){jQuery('#fb-new-required').prop('checked',true);}else{jQuery('#fb-new-required').prop('checked',false);}
if('select'===thisType||'radio'===thisType||'checkbox-multiple'===thisType){const thisOptions=fbForm.fields[id].options;jQuery('#fb-options').show();jQuery('#fb-new-options').html('');for(let i=0;i<thisOptions.length;i++){if(thisOptions[i]!==undefined){if(thisType==='radio'){jQuery('#fb-new-options').append('<div id="fb-option-box-'+
i+'" class="fb-new-fields"><span optionid="'+
i+'" class="fb-remove-option"></span><label></label><input type="text" id="fb-option'+
i+'" optionid="'+
i+'" value="'+
FB.esc_attr(fbForm.fields[id].options[i])+'" class="fb-options" /><div>');}else{jQuery('#fb-new-options').append('<div id="fb-option-box-'+
i+'" class="fb-new-fields"><span optionid="'+
i+'" class="fb-remove-option"></span><label></label><input type="text" id="fb-option'+
i+'" optionid="'+
i+'" value="'+
FB.esc_attr(fbForm.fields[id].options[i])+'" class="fb-options" /><div>');}}}}
...
function removeOption(optionId){try{const thisId=jQuery('#fb-field-id').val();const thisVal=jQuery('#fb-option'+optionId).val();const thisType=jQuery('#fb-new-type').val();jQuery('#fb-option-box-'+optionId).remove();if(thisType==='radio'){jQuery('#fb-radio-'+thisId+'-'+optionId).remove();}else if('checkbox-multiple'===thisType){jQuery('#fb-checkbox-multiple-'+thisId+'-'+optionId).remove();}else{jQuery('#fb-'+thisId+'-'+optionId).remove();}
const idx=fbForm.fields[thisId].options.indexOf(thisVal);if(idx!==-1){fbForm.fields[thisId].options.splice(idx,1);}}catch(e){if(debug){console.log('removeOption(): '+e);}}}
function removeOptions(){try{const thisId=jQuery('#fb-field-id').val();jQuery('#fb-options').hide();if(optionsCache[thisId]===undefined){optionsCache[thisId]={};}
...
function sendShortcodeToEditor(){try{jQuery('div#sortable div.fb-new-fields').each(function(index){const thisId=jQuery(this).attr('fieldid');fbForm.fields[thisId].order=index;});jQuery.post(ajaxurl,fbForm,function(response){let isVisual=jQuery('#edButtonPreview',window.parent.document).hasClass('active');if(!isVisual){isVisual=jQuery('#wp-content-wrap',window.parent.document).hasClass('tmce-active');}
const win=window.dialogArguments||opener||parent||top;let currentCode;if(isVisual){currentCode=win.tinyMCE.activeEditor.getContent();}else{currentCode=jQuery('#editorcontainer textarea',window.parent.document).val();if(typeof currentCode!=='string'){currentCode=jQuery('.wp-editor-area',window.parent.document).val();}}
const regexp=new RegExp('\\[contact-form\\b.*?\\/?\\](?:[\\s\\S]+?\\[\\/contact-form\\])?');response=response.replace(/\n/g,' ');response=response.replace(/%26#x002c;/g,',');if(currentCode.match(regexp)){if(isVisual){win.tinyMCE.activeEditor.execCommand('mceSetContent',false,currentCode.replace(regexp,response));}else{jQuery('#content',window.parent.document).val(currentCode.replace(regexp,response));}}else{try{win.send_to_editor(response);}catch{if(isVisual){win.tinyMCE.activeEditor.execCommand('mceInsertContent',false,response);}else{jQuery('#content',window.parent.document).val(currentCode+response);}}}
...
function showAndHideMessage(message){try{const newMessage=!message?GrunionFB_i18n.savedMessage:message;jQuery('#fb-success').text(newMessage);jQuery('#fb-success').slideDown('fast');setTimeout(function(){jQuery('#fb-success').slideUp('fast');},2500);}catch(e){if(debug){console.log('showAndHideMessage(): '+e);}}}
function switchTabs(whichType){try{if(whichType==='preview'){if(!validateEmails(jQuery('#fb-field-my-email').val())){return;}
...
function validateEmails(emails){if(0===emails.length){return true;}
let $e,emailList=emails.split(',');for($e=0;$e<emailList.length;$e++){if(false===validateEmail(emailList[$e])){alert(emailList[$e]+GrunionFB_i18n.invalidEmail);return false;}}
return true;}
function validateEmail(email){const re=/^(?=[a-z0-9@.!#$%&'*+/=?^_`{|}~-]{6,254}$)(?=[a-z0-9.!#$%&'*+/=?^_`{|}~-]{1,64}@)[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:(?=[a-z0-9-]{1,63}\.)[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+(?=[a-z0-9-]{1,63}$)[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/i;
