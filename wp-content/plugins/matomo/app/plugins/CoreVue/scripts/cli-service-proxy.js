...
function interceptWrite(originalWrite){return function(chunk,encoding,cb){if(typeof chunk==='string'&&/tsl.*?ERROR/.test(chunk)){chunk=chunk.replace(/(\/(?:.(?!\())+.)\((\d+)(,\d+\))/g,(m,file,line,rest)=>{if(/.vue.ts$/.test(file)){file=file.substring(0,file.length-3);const middleLine=getMiddlePosition(file);return`${file}(${middleLine + parseInt(line, 10)}${rest}`;}
return m;});}
